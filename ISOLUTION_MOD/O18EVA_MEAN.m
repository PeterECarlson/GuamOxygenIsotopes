function output = O18EVA_MEAN(tmax, TC, pCO2cave, h, V, R18_hco_ini, R18_h2o_ini, R18v, HCOMIX, h2o_new, R13_hco_ini);
...
if tmax > floor(h2o_ini/eva)
    ...
    disp('DRIPINTERVALL IS TO LONG, THE WATERLAYER EVAPORATES COMPLETLY')
end
...
h2o(end+1) = h2o_new;
delta_1 = [];
delta_1(end+1) = H2O(end)/1000/0.001;

for i=1:length(t)
    
    delta_1(end+1,1) = H2O(end)/1000/0.001;
    delta = H2O(end)/1000/0.001;
    
    %Neue Wassermenge
    h2o(end+1,1) = h2o(end) - eva*dt;                                   %Water (mol)
    ...
    HCO(end+1,1) = HCO_temp * (H2O(end-1)/H2O(end));              %HCO3- concentration after timeintervall dt and the evaporation of water
...
output = [r_hco18, r_h2o18, hco, h2o, r_hco13, delta_1];
